...
import requests # used to get the data from a website
import json
import os
import re
from bs4 import BeautifulSoup # used to parse the HTML of a website used for fetching the DOI from the meta tag
...
DOMAIN_SCORES: Dict[str, float] = {
    ...
    "arxiv.org": 0.62,          # preprint: NOT peer reviewed
    "biorxiv.org": 0.55,        # preprint: NOT peer reviewed
    # Reference
    "wikipedia.org": 0.65,
    "britannica.com": 0.80,
    # Mainstream press
...
def _openalex_signal(doi: str) -> Optional[Signal]:
    """
    Given a DOI, query OpenAlex for citation/retraction data and return the
    resulting Signal, or None if the DOI isn't indexed or the call fails.
    
    """
    try:
        response = requests.get(f"https://api.openalex.org/works/https://doi.org/{doi}", timeout=5)
        if response.status_code == 404:
            return None
        data = response.json()
        fwci = data.get("fwci", None)
        retracted = data.get("is_retracted", False)
    except Exception:
        return None
    ...
    if fwci is None:
        return None
    if retracted:
        return Signal("retracted", -0.50, f"DOI {doi} is retracted, indicating low credibility")
    elif fwci < 0.5:
        return Signal("low_impact", -0.10, f"DOI {doi} has below-average field-normalized impact (FWCI {fwci:.2f})")
    elif fwci < 1.5:
        return Signal("average_impact", 0.0, f"DOI {doi} has typical field-normalized impact (FWCI {fwci:.2f})")
    else:
        return Signal("high_impact", 0.08, f"DOI {doi} has high field-normalized impact (FWCI {fwci:.2f})")


def _fetch_doi_from_page(url: str) -> Optional[str]:
    """
    Fetches the page and looks for a citation_doi meta tag to account for URLs
    that don't contain a DOI in the URL itself. This addresses defect #1
    (the rule layer never reads the page) in a scoped, generalizable way aimed at improving
    credibility scoring for scholarly articles.

    """
    try:
        response = requests.get(url, timeout=5, headers={"User-Agent": "Mozilla/5.0"})
        soup = BeautifulSoup(response.text, "html.parser")
        tag = soup.find("meta", {"name": "citation_doi"})
...
def rule_based_signals(url: str) -> List[Signal]:
    ...
    # Signal 2edu: a .edu domain is usually credible, but not always. A student homepage on a
    # university server is not the same and its credibility shouldn't be as high.
    # Determined the a score of 0.4 is appropriate for this signal, as it is still a .edu domain
    # but falls under personal blog / user-generated content. Making similar to those scores.
    if domain.endswith(".edu") and "/~" in parsed.path:
                signals.append(Signal("edu_student_homepage", -0.42, "URL suggests student homepage on a .edu domain, " \
                "not the same as a university publication"))
    ...
    # Signal 6: OpenAlex citation and retraction for URLs that contain DOI in itself.
    # Addressed defect #2 (hand-written domain table) and #4 (retraction handling) from the KNOWN WEAKNESSES list.
    doi_match = re.search(r"(10\.\d{4,9}/\S+)", path)
    if doi_match:
        doi = doi_match.group(1)
        signal = _openalex_signal(doi)
        if signal:
            signals.append(signal)

    # Signal 7: OpenAlex citation and retraction for URLs that don't contain DOI in itself.
    # Reads the page and fetches for the citation_doi meta tag. Addressed defect #1 (the rule layer never reads the page) from the KNOWN WEAKNESSES list.
    # Aimed to improve JAMA example, which is a scholarly article that doesn't have a DOI in the URL itself, but has a citation_doi meta tag in the HTML.

    elif not doi_match:
        page_doi = _fetch_doi_from_page(url)
        if page_doi:
            match = re.match(r"(10\.\d{4,9}/\S+)", page_doi)
            if match:
                signal = _openalex_signal(match.group(1))
                if signal:
                    signals.append(signal)
...
def llm_opinion(url: str) -> Optional[Signal]:
    ...
    try:
        ...
        print(f"LLM Call failed for {url}: {type(e).__name__} {e}")
        return None
...
def score_url(url: str, use_llm: Optional[bool] = None) -> Dict[str, Any]:
    ...
    result = {"score": final, "explanation": credibility_explanation(signals + ([llm] if llm is not None else []))}
...
def credibility_explanation(signals: List[Signal]) -> str:
    """
    Addresses Defect #10 in the KNOWN WEAKNESSES list:
    Turn a list of signals into a clear readable sentence instead of semicolon-joined
    fragment lists. Giving the user a clear explanation of why a source should be trusted or not,
    a purpose-written sentence is important for transparency and trust in the scoring system.
    The baseline implementation is deliberately weak: it just joins the signal reasons with
    semicolons.
    """
    if not signals:
        return "No information available about this source."
    base = signals[0]
    explanation = base.reason.capitalize() + "."

    # HTTPS/ no_https signals are weak and not very informative, so we filter them out of the explanation
    # as they dont provide user with much insight into the credibility of the source. 
    # The other signals are more informative and relevant to the credibility assessment.
    others = [s for s in signals[1:] if s.name not in ("https", "no_https")]

    # Purpose-written, reader-facing explanations for the signals that matter
    # most to a trust decision — written to say why, not just what fired.
    explanations_by_signal = {
        "retracted": lambda s: "This work has been formally retracted, meaning its findings should not be relied upon.",
        "llm": lambda s: f"While this domain lacks a specific rule in our lookup table, an AI model assessment adds broader context for more accurate credibility judgement: {s.reason}",
        "high_impact": lambda s: f"This specific work is cited far more than average for its field ({s.reason.split('FWCI ')[-1].rstrip(')')}× the typical rate), indicating strong influence and scrutiny within the research community.",
        "low_impact": lambda s: "This specific work has below-average citation impact for its field, suggesting it has drawn limited attention or scrutiny from other researchers.",
        "edu_student_homepage": lambda s: "This appears to be a personal or student homepage rather than an official institutional publication, so it should not be treated as carrying a university's editorial credibility.",
        "path": lambda s: f"The page's URL structure suggests {s.reason.split('contains ')[-1]}, which is often associated with less rigorously edited content.",
    }

    priority_order = ["retracted","llm", "high_impact", "low_impact", "edu_student_homepage", "path"]

    for name in priority_order:
        match = next((s for s in others if s.name == name), None)
        if match and abs(match.value) >= 0.1: # Avoids returning the base raw score, would confuse users
            return explanations_by_signal[name](match)

    # Fallback to base credibility score if not major penalization or enhancement signals are present
    explanation = base.reason.capitalize() + "."
    if others:
        for name in priority_order:
            match = next((s for s in others if s.name == name), None)
            if match:
                explanation += " " + explanations_by_signal[name](match)
                return explanation
